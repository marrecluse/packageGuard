...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    nameController.text = userData['Name'] ?? ''; // Use '??' to provide a default value if 'Name' is null
    emailController.text = userData['Email'] ?? '';
    passwordController.text = ''; // You might want to handle this differently
    confirnPassController.text = ''; // You might want to handle this differently
    cellPhoneController.text = userData['Cell phone'] ?? ''; // Adjust this to match your data
    addressController.text = userData['Address'] ?? ''; // Adjust this to match your data
    address2controller.text = userData['Address 2'] ?? ''; // Adjust this to match your data
    citycontroller.text = userData['City'] ?? ''; // Adjust this to match your data
    statecontroller.text = userData['State'] ?? ''; // Adjust this to match your data
    zipcontroller.text = userData['Zip code'] ?? ''; // Adjust this to match your data
    countrycontroller.text = userData['Country'] ?? ''; // Adjust this to match your data


    print( "ssssss: ${nameController.text}" );
  ...
  Future<void> updateUserProfile(Map<String, dynamic> updatedData) async {
    ...
    final uuid = userUidController.uid.value; // Access the value inside RxString

    try {
      await FirebaseFirestore.instance.collection('users').doc(uuid).update(updatedData);
      final userDoc = await FirebaseFirestore.instance.collection('users').doc(uuid).get();
      if (userDoc.exists) {
